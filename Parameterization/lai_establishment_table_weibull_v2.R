...
# plot_leaf_area <- read.csv("seedlings_with_leaf_area_max1.csv")

spp_to_use2 <- spp_to_use[which(spp_to_use %in% names(plot_leaf_area))]
# spp_to_use2 <- names(plot_leaf_area)[3:22]
...
species_bounds[species_bounds$Species %in% c("PIRE"), ]$upper <- list(c(1, Inf, Inf, 20))
...
c <- 0
d <- 
dat <- weibull_models$data[11][[1]]
ggplot(data = dat, aes(x = lai, y = prop_present)) + 
  geom_point() + 
  ggtitle(label = sp) +
  geom_line(data = data.frame(pred = (((a/b) * ((newdat$lai/b)^(a-1)) * exp(- (newdat$lai/b)^a)) + c) * d,
                              lai = newdat$lai),
            aes(y = pred, x = lai))
