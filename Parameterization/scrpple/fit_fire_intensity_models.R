...
col_types <- list(
  ...
  tm_hardwood_biomass = col_double(),
  tm_total_biomass = col_double(),
  FFMC = col_double(),
  DMC = col_double(),
  DC = col_double(),
  ISI = col_double(),
...
data_all2 <- fire_severity_data %>%
  purrr::map_df(~read_csv(., col_types = col_types) %>%
                  ...
                  slice_sample(n = 5000)) %>%
  dplyr::filter(dnbr > 0) %>%
  dplyr::mutate(fine_fuel = ifelse(fine_fuel > 100, 1, fine_fuel/1000)) %>%
  dplyr::mutate(ladder_fuel = ifelse(ladder_fuel > 1250, 1250, ladder_fuel)) %>%
  dplyr::mutate(FWI = FWI * 19.74 / 5.38, #calibration to move from MERRA2 to LANDIS internal calculation
                ews = ews * 18.94/5.32) %>% #calibration to match LANDIS internal calculations
...
plot(dnbr ~ ews, data = data_all2)
summary(lm(dnbr~ladder_fuel, data = data_all2[data_all2$FWI <20, ]))
abline(coef(lm(dnbr~ews, data = data_all2[data_all2$FWI <20, ])))
...
test <- lmer(dnbr ~ ews+FWI*fine_fuel + (1|fire_name),
             data = data_all)
...
test <- lm(dnbr ~ ews + fine_fuel + FWI + pet, 
           data = data_all2) #pretty good!
# 
# test <- lm(dnbr ~ scale(ews) + scale(ladder_fuel) + scale(FWI) + scale(pet), 
#            data = data_all) #pretty good too! PET not significant
# 
# test <- lmer(dnbr ~ scale(ews) + scale(tm_hardwood_biomass) + scale(FWI) + scale(clay)
#              + (1|fire_code), 
#            data = data_all)
...
test <- glm(dnbr ~ ews + fine_fuel + FWI, 
            family = gaussian(link = "log"),
            data = data_all)
summary(test)
plot(allEffects(test))
plot(exp(predict(test)) ~ test$model$dnbr)
abline(0,1)
plot(test)
hist(resid(test))
hist(exp(predict(test)), xlim = c(0,1200))

test <- glm(dnbr ~ scale(ews) + scale(fine_fuel) + scale(FWI), 
            family = Gamma(link = "inverse"),
            data = data_all)
summary(test)
plot(test)
hist(resid(test))
hist(1/predict(test), xlim = c(0,1200))
plot(allEffects(test))
...
test_lmer <- lmer(I(1/dnbr) ~  FWI + ews +  fine_fuel +
