...
# 
# region_temp <- sf::st_transform(region, crs(treemap))
# 
# treemap <-  terra::crop(treemap, vect(region_temp)) %>%
...
#   group_by(tl_id) %>%
#   summarize(ladder_fuels = sum(biomass_density, na.rm = TRUE))
# hardwood_fuels <- tl_trees %>%
#   left_join(dplyr::select(sp_ref, SPCD, SFTWD_HRDWD), by = "SPCD") %>%
#   filter(SFTWD_HRDWD == "H") %>%
#   mutate(biomass = exp(-2.0127 + 2.4243*log(DIA*2.54))) %>% #Jenkins equation for Quercus rubra
#   mutate(biomass_density = biomass * TPA_UNADJ * 2.47 * 0.1) %>% #expand to kg/ac, convert to kg/ha, convert to g/m2
#   group_by(tl_id) %>%
#   summarize(hardwood = sum(biomass_density, na.rm = TRUE))
# total_biomass <- tl_trees %>%
#   mutate(biomass = exp(-2.0127 + 2.4243*log(DIA*2.54))) %>% #Jenkins equation for Quercus rubra
#   mutate(biomass_density = biomass * TPA_UNADJ * 2.47 * 0.1) %>% #expand to kg/ac, convert to kg/ha, convert to g/m2
#   group_by(tl_id) %>%
#   summarize(biomass = sum(biomass_density, na.rm = TRUE))
# 
# tm_ladder_fuel <- terra::classify(treemap, rcl = ladder_fuels, others = 0)
# writeRaster(tm_ladder_fuel, "./Parameterization/Parameterization data/treemap_ladder_fuels.tif", overwrite = TRUE)
# tm_hardwood_biomass <- terra::classify(treemap, rcl = hardwood_fuels, others = 0)
# writeRaster(tm_hardwood_biomass, "./Parameterization/Parameterization data/treemap_hardwood_biomass.tif", overwrite = TRUE)
# tm_total_biomass <- terra::classify(treemap, rcl = total_biomass, others = 0)
# writeRaster(tm_total_biomass, "./Parameterization/Parameterization data/treemap_total_biomass.tif", overwrite = TRUE)
# 

#already preprocessed
tm_ladder_fuel <- read_stars("./Parameterization/Parameterization data/treemap_fuels.tif")
tm_hardwood_biomass <- read_stars("./Parameterization/Parameterization data/treemap_hardwood_biomass.tif")
tm_total_biomass <- read_stars("./Parameterization/Parameterization data/treemap_total_biomass.tif")
...
slope_full <- read_stars("./Parameterization/Parameterization data/topography/midwest_slope.tif")

aspect_full <- read_stars("./Parameterization/Parameterization data/topography/midwest_aspect.tif")
...
#-------------------------------------------------------------------------------
daily_perims_all <- sf::st_read("./Parameterization/Parameterization data/geomac_data/geomac_combined_2000-2023.gpkg") %>%
  sf::st_as_sf() %>%
...
normal_format <- which((substr(daily_perims_all$perimeterd, 1, 4) %in% c(2000:2023)))
different_format <- which(!(substr(daily_perims_all$perimeterd, 1, 4) %in% c(2000:2023)))
...
get_raster_cell_date <- function(dnbr_raster, boundary){ 
  ...
  if(max(perims_sub$gisacres) / min(perims_sub$gisacres) < 0.33){
    use_daily_perims <<- FALSE
    message("Not enough variation in perimeters to calculate daily growth")
    return(NA)
  }
...
get_landfire_fuel <- function(boundary, dnbr_raster){
  ...
  if(fire_year > 2001){ #because 2001 data includes 2001 fire scars, 
    ...
    if(fire_year<= 2012){
      fuel <- rast("D:/Data/Landfire fuels/conus/US_105fbfm40_09122104/US_105fbfm40/grid1/us_105fbfm40") %>%
        crop(boundary) %>%
        mask(boundary)
    }else if(fire_year <= 2014){ #for 2013 and 2014 fires, use 2012 fuels
      fuel <- rast("D:/Data/Landfire fuels/conus/US_130FBFM40_05042015/US_130FBFM40/Grid2/us_130fbfm40") %>%
        crop(boundary) %>%
        mask(boundary)
    }else if(fire_year <= 2019){ #for fires in 2015 through 2019, use 2014 fuels
      fuel <- rast("D:/Data/Landfire fuels/conus/US_140FBFM40_20180618/Grid/us_140fbfm40") %>%
        crop(boundary) %>%
        mask(boundary)
    }else if(fire_year == 2020){
      fuel <- rast("D:/Data/Landfire fuels/conus/LF2019_FBFM40_200_CONUS/Tif/LC19_F40_200.tif") %>%
        crop(boundary) %>%
        mask(boundary)
    }else if(fire_year >= 2021){
      fuel <- rast("D:/Data/Landfire fuels/conus/LF2020_FBFM40_200_CONUS/Tif/LC20_F40_200.tif") %>%
        crop(boundary) %>%
        mask(boundary)
    }
    
    landfire_fine <- terra::classify(fuel, dplyr::select(fuel_lookup, VALUE, X10hr))* 224.17 * fine_fuels_coef
    landfire_ladder <- terra::classify(fuel, dplyr::select(fuel_lookup, VALUE, Live_woody))* 224.17 * ladder_fuels_coef
  
    
    landfire_fine <- as(landfire_fine, "SpatRaster") %>%
      terra::project(dnbr_raster) %>%
      terra::values()
    landfire_ladder <- as(landfire_ladder, "SpatRaster") %>%
      terra::project(dnbr_raster)%>%
      terra::values()
    
    combined <- data.frame(fine_fuel = landfire_fine,
                           ladder_fuel = landfire_ladder) #TODO why do the names stay unchaged?
...
#-------------------------------------------------------------------------------
# Trim region-wide rasters (slope, aspect, departure)
#-------------------------------------------------------------------------------
...
download_clay <- function(dnbr_raster, label){
  ...
  ssurgo_test <- get_ssurgo(template = raster::raster(dnbr_raster), 
                            label = label, 
                            raw.dir = "D:/Data/isro_ssurgo/RAW",
                            extraction.dir = paste0("D:/Data/isro_ssurgo/EXTRACTIONS/", label, "/SSURGO"),
                            force.redo = FALSE)
...
calculate_cffdrs <- function(boundary, dates_of_fire){
  ...
  fwi_data <- fwi_data %>%
    mutate(across(.cols = c(temp, rh, ws, prec), ~replace_na(.,0)))
...
create_data_catcher <- function(data_length){
  return(data.frame(fire_code = character(data_length),
                    ...
                    tm_hardwood_biomass = numeric(data_length),
                    tm_total_biomass = numeric(data_length),
...
#fires with fuels and daily progressions (year 2001) start at 126
for(i in 288:length(mtbs_shape)){
  ...
  if(length(as.vector(sf::st_area(sf::st_intersection(boundary, region)))) == 0){
    message("Fire outside of region border; moving to next fire")
    next()
  }else if(sum(sf::st_area(sf::st_intersection(boundary, region))) < sum(sf::st_area(boundary))){
    message("Fire boundary partially outside region border; setting boundary to intersection")
    boundary$geometry <- tryCatch(
      {
        suppressWarnings(sf::st_intersection(boundary, region) %>% st_union())
      },
      error = function(cond) {
  ...
  tm_fuel <-  tryCatch(
    {
      message("Extracting TreeMap fuels")
      ...
      if(as.numeric(format(fire_date, "%Y")) > 2014){
        ...
        ladder <- tm_ladder_fuel[boundary]
        hardwood<- tm_hardwood_biomass[boundary]
        total_bio<- tm_total_biomass[boundary]
        tm_fuel <- c(ladder, hardwood, total_bio)
  ...
  data <- data.frame(fire_code = rep(label, sum(cells_burned)),
                     ...
                     tm_ladder_fuel = if(sum(!is.na(tm_fuel)>1)){ tm_fuel[, 1][cells_burned]},
                     tm_hardwood_biomass = if(sum(!is.na(tm_fuel)>1)){ tm_fuel[, 2][cells_burned]},
                     tm_total_biomass = if(sum(!is.na(tm_fuel)>1)){ tm_fuel[, 3][cells_burned]},
...
write.csv(data_catcher, paste0("./Parameterization/Parameterization data/fire severity/data_catcher ", 
                               format(Sys.time(), format = "%y-%m-%d %H-%M-%S"), ".csv"))
